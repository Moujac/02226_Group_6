...
def calculate_worst_case_delay(stream, shaped_queue, link_rate, topology):
    total_delay = 0
    for i in range(len(stream['path'])-2):
        node = stream['path'][i]
...
def main():
    topology, shaped_queue = read_topology('./Small_Case/small-topology.csv')
